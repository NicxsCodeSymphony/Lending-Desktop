...
export default function EditCustomerModal({ isOpen, onClose, onCustomerUpdated, customer }: EditCustomerModalProps) {
  ...
  const [formData, setFormData] = useState<Omit<EditCustomer, 'updated_at'>>({
    customer_id: 0,
    first_name: '',
    middle_name: '',
    last_name: '',
  ...
  const [errors, setErrors] = useState<Partial<Omit<EditCustomer, 'customer_id' | 'updated_at'>>>({})
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        birthdate: customer.birthdate.split('T')[0], // Format date for input
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Omit<EditCustomer, 'customer_id' | 'updated_at'>> = {}
  ...
  const handleInputChange = (field: keyof Omit<EditCustomer, 'updated_at'>, value: string) => {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Update customer information below</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-900 font-mono">{customer.customer_id}</span>
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
              ...
              <div>
                <span className="font-medium">Last Updated:</span> {new Date(customer.updated_at).toLocaleDateString()}
              </div>
            </div>
          </div>
